from fault_injection.fake_quant_QuaRot import NoisyW8A8Conv2d, W8A8Conv2d, W8A8Linear, W8A8BMM, NoisyW8A8Linear, NoisyW8A8BMM, NoisyW8A8BMM, NoisyW8A8LinearProtected, NoisyW8A8Conv2dProtected
...
def modify_llama_attention(module, weight_quant, act_quant, quantize_bmm_input, err_prob, protected=False, method='none'):
    if protected:
        ...
        module.o_proj = NoisyW8A8LinearProtected.from_float(module.o_proj, weight_quant=weight_quant, 
                    act_quant=act_quant, quantize_output=False, err_prob=err_prob, method='AD+QuaRot')
    else:
...
class FIassistant:
    ...
    def inject_fault_to_module(self, target="", weight_quant='per_channel', act_quant='per_token', quantize_bmm_input=False, err_prob=0.0, target_layers=[], protected=False, method='none'):
        ...
        if target == "planner_all":
            self.inject_fault_to_module('planner_llama_all', 
                weight_quant=weight_quant, act_quant=act_quant, quantize_bmm_input=quantize_bmm_input, err_prob=err_prob, target_layers=target_layers, protected=protected, method=method)
            ...
            args = [
                (m, weight_quant, act_quant, quantize_bmm_input, err_prob, protected, method)
            ...
            args = [
                (m, weight_quant, act_quant, err_prob, protected, method)
                for name, m in self.model.named_modules()
        ...
        elif target == 'planner_llama_attention_o_proj':
            if method == 'quarot':
                method = 'QuaRot'
            ...
            with mp.Pool(processes=5) as pool:
                modified_modules = pool.starmap(modify_llama_attention_o_proj, args)
            ...
            rprint("injected: all llama attention o_proj layers")
                ...
                if isinstance(m, ImpalaCNN):
                    ...
                    for i in target_layers: #0,1,2
                        if not protected:
                            ...
                            # print(target_layers)
                            m[i].firstconv.layer = NoisyW8A8Conv2dProtected.from_float(
                                m[i].firstconv.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            m[i].blocks[0].conv0.layer = NoisyW8A8Conv2dProtected.from_float(
                                m[i].blocks[0].conv0.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            m[i].blocks[0].conv1.layer = NoisyW8A8Conv2dProtected.from_float(
                                m[i].blocks[0].conv1.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            m[i].blocks[1].conv0.layer = NoisyW8A8Conv2dProtected.from_float(
                                m[i].blocks[1].conv0.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            m[i].blocks[1].conv1.layer = NoisyW8A8Conv2dProtected.from_float(
                                m[i].blocks[1].conv1.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
             ...
             for name,m in self.model.named_modules():
                if isinstance(m, ImgObsProcess): #不分层
                    ...
                    if not protected:
                        ...
                        m.cnn.dense.layer = NoisyW8A8LinearProtected.from_float(
                            m.cnn.dense.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)    
                        
                        m.linear.layer = NoisyW8A8LinearProtected.from_float(
                            m.linear.layer, weight_quant=weight_quant, act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                ...
                if isinstance(m, ResidualRecurrentBlocks):
                    ...
                    for i in target_layers:
                        ...
                        if not protected:
                            ...
                            q.mlp0.layer = NoisyW8A8LinearProtected.from_float(q.mlp0.layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            q.mlp1.layer = NoisyW8A8LinearProtected.from_float(q.mlp1.layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            q.r.orc_block.q_layer = NoisyW8A8LinearProtected.from_float(q.r.orc_block.q_layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            q.r.orc_block.k_layer = NoisyW8A8LinearProtected.from_float(q.r.orc_block.k_layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            q.r.orc_block.v_layer = NoisyW8A8LinearProtected.from_float(q.r.orc_block.v_layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            q.r.orc_block.proj_layer = NoisyW8A8LinearProtected.from_float(q.r.orc_block.proj_layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                            q.r.orc_block.r_layer = NoisyW8A8LinearProtected.from_float(q.r.orc_block.r_layer, weight_quant=weight_quant,  
                                act_quant=act_quant, quantize_output=quantize_bmm_input, err_prob=err_prob, method=method)
                    '''for i in [0, 1, 2, 3]:
                        q = m.blocks[i]
