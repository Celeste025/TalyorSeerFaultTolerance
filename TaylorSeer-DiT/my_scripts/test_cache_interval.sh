...
ERR_PROB_LIST=(3e-3 1e-2 1e-3)
TARGET_LIST=("DiTXL512full-step2t49")  ###保护前2step
TARGET_LAYERS_LIST=("")
PROTECT_LIST=("ABFT_8")
CACHE_ORDER_LIST=(-1)
CACHE_QUANT_LIST=(8)        # ✅ cache_quant 参数列表
CACHE_INTERVAL_LIST=(1)   # ✅ 新增 cache_interval 参数列表
            ...
            for target_layers in "${TARGET_LAYERS_LIST[@]}"; do
                for protect in "${PROTECT_LIST[@]}"; do
                    for cache_order in "${CACHE_ORDER_LIST[@]}"; do
                        for cache_quant in "${CACHE_QUANT_LIST[@]}"; do
                            for cache_interval in "${CACHE_INTERVAL_LIST[@]}"; do  # ✅ 新增循环
                                ...
                                echo "  cache_quant: $cache_quant"
                                echo "  cache_interval: $cache_interval"
                                echo "=============================================="
                                
                                # 构建命令
                                CMD="$BASE_CMD --num_inference_steps $num_steps --err_prob $err_prob --target $target --protect $protect --cache_order $cache_order --cache_quant $cache_quant --cache_interval $cache_interval"
