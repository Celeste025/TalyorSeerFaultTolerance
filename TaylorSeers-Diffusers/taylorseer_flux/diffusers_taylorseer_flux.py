...
def make_result_folder_name(target: str, num_inference_steps: int, err_prob: float, target_layers, bit, protect, additional_str=None) -> str:
    folder_name = f"Debug2_target_{target}_step_{num_inference_steps}_err_prob_{err_prob}"
...
def main():
    ...
    if (args.analyzer_yes or args.hook_yes) and (args.max_prompts != 1):
        print("Warning. It is strongly recommended to set max_prompts to 1 when using analyzer_yes and hook_yes")
    additional_str = ""
    ...
    folder = make_result_folder_name(args.target, args.num_inference_steps, args.err_prob, args.target_layers, args.bit, args.protect, additional_str=additional_str)
    os.makedirs(folder, exist_ok=True)
    ...
    if args.hook_yes:
        ...
        _hook_manager.register_hooks(capture_mode="both", print_module_names=False)
    #######
