...
def taylorseer_flux_forward(
    ...
    if _injection_state.global_args['hook_yes']:
        hook_save_dir = os.path.join(_injection_state.global_args['folder_path'], "layer_inout")
    ...
    if not return_dict:
        return (output,)
    print("!Waring! please check flux forward to see if injection_step are set correctly.")
    final_result = Transformer2DModelOutput(sample=output)
